                ...
                <div class="yt_ai_summary_header_actions">
                    ...
                    <div id="yt_ai_summary_header_track" class="yt_ai_summary_header_action_btn yt-summary-hover-el" data-hover-label="Jump to Current Time">
                        <svg style="filter: brightness(0.9);" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="12" cy="12" r="6.25" stroke="#FFFFFF" stroke-width="1.5"/>
                            <rect x="3.19995" y="11.3999" width="5" height="1.2" rx="0.6" fill="#FFFFFF"/>
                            <rect x="15.7" y="11.3999" width="5" height="1.2" rx="0.6" fill="#FFFFFF"/>
                            <rect x="11.3999" y="8" width="5" height="1.2" rx="0.6" transform="rotate(-90 11.3999 8)" fill="#FFFFFF"/>
                            <rect x="11.3999" y="21" width="5" height="1.2" rx="0.6" transform="rotate(-90 11.3999 21)" fill="#FFFFFF"/>
                        </svg>
                    </div>
                    <div id="yt_ai_summary_header_copy" class="yt_ai_summary_header_action_btn yt-summary-hover-el" data-hover-label="Copy Transcript\n(Plain Text)">
                        <svg style="filter: brightness(0.95);" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7 6.6V5C7 4.44772 7.44772 4 8 4H18C18.5523 4 19 4.44772 19 5V16C19 16.5523 18.5523 17 18 17H16.2308" stroke="#FFFFFF" stroke-width="1.5"/>
                            <rect x="4.75" y="6.75" width="11.5" height="13.5" rx="1.25" stroke="#FFFFFF" stroke-width="1.5"/>
                        </svg>
                    </div>
...
// ------------------------------------------------------------------------------------------------------------///////
            ...
            // // Show the token input div regardless of the value returned by getApiKeyFromUser()
            // document.querySelector("#yt_ai_summary_body").insertAdjacentHTML("beforeend", `
            //     <div id="tokenInputDiv" style="display: block;">
            //         <label for="apiKey">Enter Token:</label>
            //         <input type="text" id="apiKey" name="apiKey">
            //         <button id="submitTokenBtn">Submit</button>
            //     </div>
            // `);
            ...
            // } else {
            //     console.log("Stopping the process as the function returned false.");
            //     return;
            // }
...
console.log("What is should proceed value is ======>", shouldProceed)
if (shouldProceed) {
    console.log("Proceeding with further processing...");
} else {
    console.log("Stopping the process as the function returned false.");
...
async function  getApiKeyFromUser() {
    ...
    return new Promise((resolve, reject) => {
        // Show the token input div
        document.querySelector("#tokenInputDiv").style.display = "block";

        // Add event listener to the submit button
        document.querySelector("#submitTokenBtn").addEventListener("click",async () => {
            const apiKey = document.querySelector("#apiKey").value.trim();

            if (apiKey) {
               
                try {
                    console.log("Before API cALL",apiKey);
                    // Make an API call to your local server
                    const response = await fetch('http://localhost:3000/verify', {
                       ...
                       console.log("After API cALL",responseData.user.id);

                    if (response.ok) {

                        saveData("User",responseData.user);

                        getData('User', function(value) {
                            console.log('Retrieved value =========== > ', value);
                          });
                          

                        // Hide the token input div
                        document.querySelector("#tokenInputDiv").style.display = "none";

                        // Resolve the promise with the entered token
                        resolve(true);
                    } else {
                ...
                // Reject the promise if the token is empty
                reject(false);
            }
        });
    });
